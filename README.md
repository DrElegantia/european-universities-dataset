...
- **Field category is institution-level**, classified from the university name (e.g. "Technical University" → Engineering & Technology; general universities → Comprehensive). It is the institution's primary character, not an exhaustive programme list. ~62% of fields are Comprehensive.
  - **Field filter semantics (important):** when the user filters by a specific field (e.g. Medicine), the app MUST also include `COMP` (comprehensive) universities, because they offer a broad range of programmes including that field. Matching only the exact `field_category` returns just the specialized single-field institutions (e.g. only "Campus Bio-Medico" under Medicine in Italy, hiding Bologna/Sapienza/Padua). Rule: a university matches field X if `field_category == X OR field_category == "COMP"`. Show a small "multidisciplinare / comprehensive" tag on COMP results. For exact per-institution programme lists, ETER (ISCED fields) would be the authoritative source (not yet integrated).
