...
- **Cost of living** is a point-in-time Numbeo estimate (accessed June 2026) plus official student-budget benchmarks; indicative, not a guaranteed budget. 66 cities have city-level data; uncovered cities use the national figure.
- **Room rent (`monthly_room_rent_eur`)** is the headline comparable metric, on 100% of universities. ~62% are city-level (530 cities), the rest use the national baseline. Source priority per city: (1) curated local sources + market portals specific to rooms (SpareRoom UK, WG-Gesucht DE, Idealista/Immobiliare IT-ES-PT, Kamernet NL, Otodom PL, LocService FR, HousingAnywhere, Daft IE) = real room prices; (2) estimate from Numbeo 1-bedroom rent × 0.6 (single-room share). `room_rent_source` + the city note say which. Comparability caveat: portal figures differ on bills-included (DE/UK warm) vs rent-only (most others); flagged per row.
